...
.. note::
  Since v1.3.0, the GUI's default QSM dipole inversion method is toolbox-availability-aware: it follows a consensus-informed priority chain (FANSI → MEDI → LSQR+HEIDI → TKD) and automatically selects the highest-priority method whose required toolbox is actually installed, rather than always defaulting to the same method.
