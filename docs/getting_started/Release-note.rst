...
Masking
^^^^^^^
* New two-pass masking option (see :ref:`method-qsm-two-pass-masking`), and a new, generalised mask refinement pipeline (shared between the I/O panel's "Refine mask" option and the QSM panel's two-pass masking)
