...
5. Use FSLView to display the local field map (Hz) when using 'Peel' value of 2 (should be in *~/qsm_tutorial/data/output_backgroundremove/*):
  ...
  #. Go to location [119 142 88] so that you can see the differences between the two maps with a better view. 
  
  #. Check/Uncheck the 'Visibility' button in the buttom of the viewer to turn on/off of the top image. What is the main difference between the two results?
...
6. Compare the location of the edges with what you can see in the mean magnitude image. (Assuming you are still in the data directory, if not then go back to the data directory *~/qsm_tutorial/data/* in the terminal) This can be computed on the terminal using:
  ...
  #. Use shortcut `Crtl+A` in the viewer to add *mag_mean.nii.gz* to the currently displayed loca field maps.

  #. Adjust the display window of *mag_mean.nii.gz* to 'Min 0' and 'Max 300'.

  #. Go to location [133 155 81] which is the top edge of the globus pallidus.

  #. Check/Uncheck the 'Visibility' button to turn on/off of the mean magnitude image.

  #. It is known that there is high concentration of iron deposition (i.e. paramagnetic substance) in the globus pallidus. Can you identify the secondary magnetic field generated by this structure?
